  ...
  <div class="min-h-screen bg-gray-100 p-6">
    <div class="max-w-3xl mx-auto">
      <div class="flex items-center gap-4 mb-6">
        ...
        <h2 class="text-xl font-bold text-gray-800">スケジュール登録</h2>
      </div>

      <div v-if="successMessage" class="bg-green-50 border border-green-300 text-green-800 px-4 py-3 rounded mb-4 text-sm">{{ successMessage }}</div>
      ...
      <div class="bg-white rounded-xl shadow p-6 mb-6">
        <p class="text-sm text-gray-500 mb-3">地名を入力して候補から選択するか、地図をクリックして出発地・目的地を選択してください</p>
        <div class="h-96 rounded overflow-hidden">
          <RouteMap :origin="origin" :dest="dest" :clickable="!submitted" :center="mapCenter" @point-selected="handlePointSelected" />
        </div>
        <div class="flex gap-4 mt-3 text-sm">
          <div class="flex-1 px-3 py-2 bg-blue-50 rounded">
            ...
            <span v-else class="text-gray-400"> 未選択</span>
          </div>
        ...
        <div v-if="origin || dest" class="mt-2">
          <button @click="resetPoints" class="text-sm text-gray-500 border border-gray-300 px-3 py-1 rounded hover:bg-gray-50">地点をリセット</button>
        </div>
      </div>

      <!-- フォーム -->
      <div class="bg-white rounded-xl shadow p-6">
        <h3 class="font-semibold text-gray-800 mb-4">スケジュール情報</h3>
        <form @submit.prevent="handleSubmit" class="space-y-4">
          <div class="grid grid-cols-2 gap-4">
            ...
            <div class="relative">
              <label class="block text-sm text-gray-600 mb-1">
                出発地名
                <span v-if="geocoding.origin" class="text-blue-600 text-xs ml-1">検索中...</span>
              </label>
              <input v-model="form.originName" type="text" required placeholder="例: 東京駅" autocomplete="off"
            ...
            <div class="relative">
              <label class="block text-sm text-gray-600 mb-1">
                目的地名
                <span v-if="geocoding.dest" class="text-blue-600 text-xs ml-1">検索中...</span>
              </label>
              <input v-model="form.destName" type="text" required placeholder="例: 大阪駅" autocomplete="off"
          ...
          <div class="grid grid-cols-2 gap-4">
            <div>
              ...
              <input v-model="form.departAt" type="datetime-local" required
                class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
            <div>
              <label class="block text-sm text-gray-600 mb-1">到着予定日時</label>
              <input v-model="form.arriveAt" type="datetime-local" required
                class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
          </div>
          ...
          <div class="grid grid-cols-2 gap-4">
            <div>
              ...
              <input v-model.number="form.maxWeightKg" type="number" min="0.1" max="10000" step="0.1" required
                class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
              <p class="text-xs text-gray-400 mt-1">バス全体の積載可能重量（例: 100kg）</p>
            </div>
            <div>
              <label class="block text-sm text-gray-600 mb-1">積載可能サイズ (cm) <span class="text-red-500 text-xs">※1個最大140cm</span></label>
              <input v-model.number="form.maxSizeCm" type="number" min="0.1" max="140" step="0.1" required
                class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                :class="form.maxSizeCm > 140 ? 'border-red-400 bg-red-50' : ''" />
              <p v-if="form.maxSizeCm > 140" class="text-red-500 text-xs mt-1">システム上限は140cmです</p>
              <p v-else class="text-xs text-gray-400 mt-1">3辺合計の上限（最大140cm）</p>
            </div>
          </div>

          <button type="submit" :disabled="loading || !origin || !dest || submitted || form.maxSizeCm > 140"
...
const debounceTimers = { origin: null, dest: null }
